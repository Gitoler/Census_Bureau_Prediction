...
xgb_model = md.train_model(X_train, y_train)
# Evaluate the model on the test data
logging.info("Start the evaluation of the model")
threshold = 0.52
# evaluate trained model on test set
predictions = md.inference(xgb_model, X_test)
recall, precision, fscore, accuracy, y_test, y_predict, predicted_data = md.evaluation(
    test, predictions,
    y_test, threshold)
logging.info(
...
cm = md.save_confusion_matrix(y_test, y_predict)
...
for feature in enumerate(cat_features):
    performance_df = md.slices_evaluation(test, feature[1], y_test, np.array(y_predict))
    if feature[0] == 0:
